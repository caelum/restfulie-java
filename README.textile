...
public class Order implements HypermediaResource {
	...
	public List<Transition> getFollowingTransitions(Restfulie control) {
		control.transition("up").at('http://caelumobjects.com');
		return control.getTransitions();
	}
...
*API CHANGE*: StateResource changed to HypermediaResource. getFollowingTransitions changed to getRelations. Transition class renamed to Relation
*API CHANGE*: Package restfulie.vraptor changed to vraptor.restfulie
