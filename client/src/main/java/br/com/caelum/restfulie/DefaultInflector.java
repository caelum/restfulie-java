...
public class DefaultInflector extends RuleBasedPluralizer 
{
	...
	private final List<Rule> rules = Arrays.asList(new Rule[] 
	{
		...
		new AbstractRegexReplacementRule("(?i)^(?:" + POSTFIX_ADJECTIVE_REGEX + ")$") 
		{
			...
			public String replace(Matcher m) {
				return DefaultInflector.this.pluralize(m.group(1)) + m.group(2);
			}
		},
		...
		new AbstractRegexReplacementRule("(?i)(.*?)((?:-|\\s+)(?:" + disjunction(PREPOSITIONS) + "|d[eu])(?:-|\\s+))a(?:-|\\s+)(.*)") 
		{
			...
			{
				return DefaultInflector.this.pluralize(m.group(1)) +
					m.group(2) +
					DefaultInflector.this.pluralize(m.group(3));
			}
		},
